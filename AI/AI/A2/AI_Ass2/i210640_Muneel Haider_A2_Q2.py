...
def fitness(matrix):
    ...
    magicSum = 15
    rowDiff = sum(abs(magicSum - sum(row)) for row in matrix)
    colDiff = sum(abs(magicSum - sum(col)) for col in zip(*matrix))
    diagonalDiff = abs(magicSum - sum(matrix[i][i] for i in range(3))) + abs(magicSum - sum(matrix[i][2-i] for i in range(3)))
    
    return rowDiff + colDiff + diagonalDiff
