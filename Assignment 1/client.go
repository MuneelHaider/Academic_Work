...
func main() {
	// Setup TLS but disable verification
	...
	fmt.Print("[CLIENT] Enter first matrix (semicolon-separated rows, space-separated values): ")
	matrixA, _ := reader.ReadString('\n')
	matrixA = strings.TrimSpace(matrixA)

	var matrixB string
	if operation != "TRANSPOSE" {
		fmt.Print("[CLIENT] Enter second matrix: ")
