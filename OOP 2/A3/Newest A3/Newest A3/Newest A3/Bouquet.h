...
public:
	...
	Flower();
	Flower(string n, string c, float, bool, bool);
	...
	friend ostream& operator<<(ostream& output, const Flower& Val) {

		output << Val.name << " " << Val.color << " " << Val.perfumed << ", Price: " << Val.UnitPrice << " Rs.";
...
public:
	...
	void increase1Amount();

	float price();
	operator string() const;

	friend ostream& operator<<(ostream& output, Bouquet& obj) {

		for (int i = 0; i < obj.amount; i++) {
			
			output << "Name:\t" << obj.group[i].getName() << endl;
			output << "Color:\t" << obj.group[i].getColor() << endl;
			output << "Perfumed:\t" << obj.group[i].getPerf() << endl;
			output << "Price:\t" << obj.group[i].getPrice() << endl;
			output << "Sale:\t" << obj.group[i].getSale() << endl;
		}

		return output;
	}

	Bouquet operator+=(Flower& obj);
